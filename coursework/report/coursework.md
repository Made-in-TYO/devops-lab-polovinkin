...
1. **Что делает MkDocs?** Преобразует Markdown и настройки из `mkdocs.yml` в статический HTML-сайт.
2. **Чем `serve` отличается от `build`?** `serve` запускает локальный предпросмотр, а `build` сохраняет готовые файлы в `site/`.
3. **Для чего нужен `mkdocs.yml`?** Для метаданных, темы, навигации, плагинов и расширений.
4. **Как добавить страницу?** Создать `.md`-файл в `docs/`, добавить его в `nav` и пересобрать сайт.
5. **Как работает поиск?** При сборке создаётся индекс, по которому JavaScript выполняет поиск в браузере.
6. **Зачем нужно виртуальное окружение?** Оно изолирует зависимости проекта от системного Python.
7. **Что делает режим `--strict`?** Завершает сборку ошибкой при предупреждениях MkDocs.
8. **Что публикуется на GitHub Pages?** Собранное содержимое `site/`, а не `.venv` и не исходные Markdown-файлы.
9. **Почему сайт доступен по HTTPS?** GitHub Pages предоставляет HTTPS для опубликованного статического сайта.
10. **Почему кейсы вымышлены?** Задание проверяет технологию и структуру сайта; реальные личные достижения публиковать не требуется.
